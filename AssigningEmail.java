public class AssigningEmail {
    public static void sendEmailmsg(List<Assignment__c> assRec){
        ...
        Map<id,Techniciancsv__c> tecnicians = new Map<id,Techniciancsv__c>([SELECT Id, Phone__c, Location__c, Skills__c, Name__c, Email__c, Availibility__c, Name FROM Techniciancsv__c]);
        try{
            for(Assignment__c con : assRec){
                if(con.Technician_csv__c != null){   
                    messaging.SingleEmailMessage mail = new messaging.SingleEmailMessage();
                    List<String> sendTo = new List<String>();
                    sendTo.add(tecnicians.Get(con.Technician_csv__c).Email__c);
